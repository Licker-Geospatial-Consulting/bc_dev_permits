...
Public API:
    ...
    harvest(session=None)                      -> list[dict]   (live; needs network)
"""
...
LIST_URL = ("https://www.cnv.org/Business-Development/Building/"
            "Land-Use-Approvals/Active-Applications")
MUNICIPALITY = "north_van"
PERMIT_ID_RE = re.compile(r"PLN\d{4}-\d{5}", re.I)
MIN_PROSE_CHARS = 120          # below this we treat the page as "no usable text"
USER_AGENT = "bc-dev-permit-scraper/0.1 (+contact: you@example.com)"
TIMEOUT = 90                   # cnv.org streams the 500KB index slowly when cold
RETRIES = 3                    # transient read/connection timeouts are common here
CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".http_cache")
CACHE_TTL = 6 * 3600           # reuse a fetched page for 6h so test re-runs are instant
...
# --------------------------------------------------------------------------- #
def _build_session():
    """A requests Session that retries transient timeouts / 5xx with backoff."""
    import requests
...
def _milestones(main) -> list[dict]:
    ...
    for table in main.find_all("table"):
        header = " ".join(th.get_text(" ", strip=True).lower()
                          for th in table.find_all(["th", "td"])[:4])
...
# --------------------------------------------------------------------------- #
# CLI — run a few examples live, then print / save JSON / upsert to Postgres.
# Run from the harvesters/ dir so `import fields` / `import load` resolve, e.g.:
#     python north_van.py --limit 3                 # eyeball 3 rows on stdout
#     python north_van.py --limit 3 --out json      # -> north_van_sample.json
#     python north_van.py --out db --dsn postgresql://user:pw@localhost/dbname
# --------------------------------------------------------------------------- #
def _run_cli(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(description="Harvest City of North Vancouver applications.")
    ap.add_argument("--limit", type=int, default=3,
                    help="Process only the first N applications (default 3; 0 = all).")
    ap.add_argument("--out", choices=("print", "json", "db"), default="print",
                    help="Where results go (default: print to stdout).")
    ap.add_argument("--json-path", default="north_van_sample.json",
                    help="Output file for --out json (default: north_van_sample.json).")
    ap.add_argument("--dsn", default=os.environ.get("DATABASE_URL"),
                    help="Postgres DSN for --out db (or set DATABASE_URL).")
    ap.add_argument("--no-cache", action="store_true",
                    help=f"Bypass the {CACHE_TTL // 3600}h page cache in {CACHE_DIR} and refetch.")
    args = ap.parse_args(argv)

    rows = harvest(limit=args.limit or None, use_cache=not args.no_cache)
    print(f"Harvested {len(rows)} application(s).", file=sys.stderr)

    if args.out == "json":
        with open(args.json_path, "w", encoding="utf-8") as fh:
            json.dump(rows, fh, indent=2, ensure_ascii=False)
        print(f"Wrote {len(rows)} row(s) -> {args.json_path}", file=sys.stderr)
    elif args.out == "db":
        if not args.dsn:
            ap.error("--out db needs --dsn or the DATABASE_URL env var.")
        import psycopg          # pip install "psycopg[binary]"
        import load             # harvesters/load.py — build_statements/upsert
