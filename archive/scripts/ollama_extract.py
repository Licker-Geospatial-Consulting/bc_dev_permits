...
HTML pages and map attributes are parsed deterministically by harvesters/fields.py and
never reach a model. This module is the fallback path for PDF documents.

Entry points (JSON contract in resources/extraction_prompt.md):
  ...
  * extract_from_text(text)  -> dict   # helper for text ALREADY pulled from a PDF
                                       # (not for HTML page prose — use fields.py)
...
TEXT_MODEL = "qwen2.5:7b-instruct"     # any solid local instruct model works
VISION_MODEL = "llama3.2-vision"       # for scanned/image-only PDFs
...
def extract_from_pdf(path: str | Path, model: str = TEXT_MODEL,
                     vision_model: str = VISION_MODEL) -> dict:
