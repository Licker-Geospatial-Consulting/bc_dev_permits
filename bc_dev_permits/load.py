"""Map a parsed harvester row to parameterized SQL for the schema in references/schema.sql.
...
def build_statements(row: dict) -> list[tuple[str, list]]:
    ...
    updates = ", ".join(
        f"{c} = EXCLUDED.{c}" for c in _PERMIT_COLS if c not in ("municipality", "permit_id")
    )
    stmts.append(
        (
    ...
    occupancies = row.get("occupancies") or [
        {"occupancy": o} for o in row.get("occupancy_types", [])
    ]
